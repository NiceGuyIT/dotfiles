_atuin() {
    ...
    case "${cmd}" in
        atuin)
            opts="-h -V --help --version history import stats search sync login logout register key server init uuid contributors gen-completions help"
        ...
        atuin__gen__completions)
            ...
            ;;
        atuin__help)
            opts="history import stats search sync login logout register key server init uuid contributors gen-completions help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        atuin__help__history__start)
            ...
            ;;
        atuin__help__import)
            opts="auto zsh zsh-hist-db bash resh fish"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        atuin__history__start)
            ...
            ;;
        atuin__import)
            opts="-h --help auto zsh zsh-hist-db bash resh fish help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        atuin__import__fish)
            ...
            ;;
        atuin__import__help)
            opts="auto zsh zsh-hist-db bash resh fish help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        atuin__import__zsh__hist__db)
            ...
            ;;
        atuin__init)
            opts="-h --disable-ctrl-r --disable-up-arrow --help zsh bash fish"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        atuin__register)
            ...
            ;;
        atuin__search)
            opts="-c -e -b -i -f -h --cwd --exclude-cwd --exit --exclude-exit --before --after --limit --interactive --filter-mode --search-mode --shell-up-key-binding --human --cmd-only --format --help [QUERY]..."
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
            case "${prev}" in
                ...
                --limit)
                    ...
                    ;;
                --filter-mode)
                    COMPREPLY=($(compgen -W "global host session directory" -- "${cur}"))
                    return 0
                    ;;
                --search-mode)
                    COMPREPLY=($(compgen -W "prefix full-text fuzzy" -- "${cur}"))
                    return 0
                    ;;
                --format)
                    COMPREPLY=($(compgen -f "${cur}"))
